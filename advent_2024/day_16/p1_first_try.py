...
class RaceCourse:
    ...
    def get_best_path_score(self) -> int:
        ...
        for i in range(2, len(path)):
            next_direction = self._get_current_direction(path[i], path[i - 1])
            if next_direction != current_direction:
                turn_count += 1
                current_direction = next_direction
        return total_forward_steps + self._turn_penalty * turn_count
    ...
    @staticmethod
    def _get_current_direction(node: GridPoint, prev: GridPoint) -> str:
        if node[0] == prev[0]:
            if node[1] - prev[1] == 1:
                return "east"
            else:
...
def _parse_race_track(
    ...
    row_count = -1
    col_count = -1
    for r, row in enumerate(lines := puzzle_input.read().splitlines()):
        row_count = len(lines)
        col_count = len(row)
