        ...
        for line in lines:
            if line == "$ ls":
                ...
                if line.split(" ")[2].strip() == "..":
                    ...
                    if "dir " + current_dir_name in tree and current_dir_name != "/":
                        raise ValueError(f"Duplicate dir name: dir {current_dir_name}")
                    current_dir_contents = set()
            else:
...
def part_one(file: str, day: int = 7, year: int = 2022):
    ...
    root_dir_size, size_map = _dfs(tree=tree, size_map=size_map, node="dir /")
    # print(f"{size_map=}")
    # print(f"{root_dir_size=}")
    total = 0
    for dir_name, size in size_map.items():
        if size <= 100000:
            total += size
    return total
